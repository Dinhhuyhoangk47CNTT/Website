...
namespace TECH.Areas.Admin.Controllers
{
    public class HopDongController : BaseController
    {
        ...
        private KhachHangModelView LoginUser;
        ...
        public JsonResult Add(HopDongModelView HopDongModelView)
        {
            ...
            // TODO
            HopDongModelView.MaNV = nhanVien.Id;

            _hopDongService.Add(HopDongModelView);
            // update trạng thái phòng thành đã có khách thuê
            if (HopDongModelView.MaPhong.HasValue 
                && HopDongModelView.MaPhong.Value > 0 
                && HopDongModelView.TrangThai.HasValue 
                && (HopDongModelView.TrangThai.Value == 1 
                    || HopDongModelView.TrangThai.Value == 2))
            {
