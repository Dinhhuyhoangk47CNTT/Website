(function ($) {
    ...
    self.Update = function (id) {
        if (id != null && id != "") {
            $(".txtPassword").hide();
            $("#titleModal").text("Cập nhật thông báo");
    ...
    self.Deleted = function (id) {
        if (id != null && id != "") {
            tedu.confirm('Bạn có chắc muốn xóa thông báo này?', function () {
                $.ajax({
                    type: "POST",
                    url: "/Admin/ThongBao/Delete",
                    data: { id: id },
                    beforeSend: function () {
                    },
                    success: function () {
    ...
    self.GetDataPaging = function (isPageChanged) {
        ...
        $.ajax({
            url: '/Admin/ThongBao/GetAllPaging',
            type: 'GET',
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    ...
                    tedu.notify('Tiêu đề thông báo đã tồn tại', 'error');
                }
            }
        })
    }
    ...
    self.ValidateUser = function () {
        $("#form-submit").validate({
            ...
            submitHandler: function (form) {
                self.GetValue();
                if (self.IsUpdate) {
                    self.UpdateUser(self.ThongBao);
                }
                else {
                    self.AddUser(self.ThongBao);
                }
            }
        });
    }
    ...
    self.GetValue = function () {
        self.ThongBao.TieuDe = $("#TieuDe").val();
        self.ThongBao.NoiDung = $("#NoiDung").val();
        self.ThongBao.NgayHetHan = $.datepicker.formatDate("yy-mm-dd", $("#NgayHetHan").datepicker("getDate"));
    }

    self.SetValue = function () {
        $("#TieuDe").val(self.ThongBao.TieuDe);
        $("#NoiDung").val(self.ThongBao.NoiDung);
        tinymce.activeEditor.setContent(self.ThongBao.NoiDung);
        $("#NgayHetHan").val(self.ThongBao.NgayHetHan);
    }
